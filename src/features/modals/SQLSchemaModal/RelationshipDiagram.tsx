...
import { FiArrowRight, FiGitCommit, FiGrid, FiCircle, FiInfo } from "react-icons/fi";
import type { TableSchema, RelationshipInfo } from "../../../store/useSQLSchema";
...
export const RelationshipDiagram: React.FC<RelationshipDiagramProps> = ({ relationships }) => {
  ...
  const renderRelationshipCard = (relationship: RelationshipInfo, index: number) => {
    const isSelected =
      selectedRelationship === `${relationship.sourceTable}-${relationship.targetTable}`;
      ...
      <Card
        ...
        style={{
          ...
          borderColor: isSelected
            ? `var(--mantine-color-${getRelationshipColor(relationship.relationshipType)}-4)`
            : undefined,
        }}
        onClick={() =>
                ...
                <Stack gap="xs">
                  <Paper p="xs" bg="var(--mantine-color-blue-0)" withBorder>
                    <Text size="xs">
                      ...
                      Table &quot;{relationship.sourceTable}&quot; references &quot;
                      {relationship.targetTable}&quot; through column &quot;
                      {relationship.foreignKeyColumn}
                      &quot;
                    </Text>
                  </Paper>
                  ...
                  {relationship.relationshipType === "ONE_TO_MANY" && (
                    <Paper p="xs" bg="var(--mantine-color-yellow-0)" withBorder>
                      <Text size="xs">
                        One record in &quot;{relationship.targetTable}&quot; can have multiple
                        records in &quot;{relationship.sourceTable}&quot;
                      </Text>
                    </Paper>
                  )}
                  ...
                  {relationship.relationshipType === "MANY_TO_MANY" &&
                    relationship.junctionTable && (
                      <Paper p="xs" bg="var(--mantine-color-orange-0)" withBorder>
                        <Text size="xs">
                          Many-to-many relationship implemented through junction table &quot;
                          {relationship.junctionTable}&quot;
                        </Text>
                      </Paper>
                    )}
      ...
      <Card withBorder bg="var(--mantine-color-gray-0)">
        ...
        <Stack gap="xs">
          <Group>
            ...
            <Text size="xs" c="dimmed">
              One-to-One: Each record in the source table corresponds to exactly one record in the
              target table
            </Text>
          </Group>
          ...
          <Group>
            ...
            <Text size="xs" c="dimmed">
              One-to-Many: Each record in the target table can have multiple related records in the
              source table
            </Text>
          </Group>
          ...
          <Group>
            ...
            <Text size="xs" c="dimmed">
              Many-to-Many: Records in both tables can have multiple relationships (requires
              junction table)
            </Text>
          </Group>
        </Stack>
