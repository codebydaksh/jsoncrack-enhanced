...
export interface MockField {
  path: string;
  type: "string" | "number" | "boolean" | "array" | "object" | "uuid" | "email" | "phone" | "name" | "address" | "company" | "lorem" | "date" | "url" | "image";
  options?: {
    min?: number;
    max?: number;
    format?: string;
...
const DEFAULT_TEMPLATES: DataTemplate[] = [
  ...
  },
  {
    ...
    mockFields: [
      ...
      { path: "category", type: "string", options: { enum: ["Electronics", "Clothing", "Books", "Home", "Sports"] } },
  ...
  },
  {
    ...
    mockFields: [
      ...
      { path: "message", type: "string", options: { enum: ["Success", "Data retrieved successfully", "Operation completed"] } },
...
const generateMockValue = (field: MockField): any => {
  ...
  switch (type) {
    ...
    case "lorem":
      return generateLorem(options.format as "word" | "words" | "sentence" | "paragraph", options.max);
...
const generateLorem = (format: "word" | "words" | "sentence" | "paragraph" = "word", count = 1): string => {
...
const useDataGeneration = create<DataGenerationState & DataGenerationActions>((set, get) => ({
  ...
  saveDataset: (dataset) => {
    set(state => ({
      recentDatasets: [dataset, ...state.recentDatasets.filter(d => d.id !== dataset.id)].slice(0, 10),
    }));
  },
    ...
    switch (format) {
      ...
      case "sql":
        ...
        const insertStatements = dataset.data.map(item => {
          const values = columns.map(col => `'${(item[col] || "").toString().replace(/'/g, "''")}'`);
